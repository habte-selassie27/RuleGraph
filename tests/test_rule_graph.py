...
CONTRACT_SOURCE = (Path(__file__).parents[1] / "contracts" / "rule_graph.py").read_text(encoding="utf-8")
...
def propose_first(direct_vm, contract, book_id, semantics=None, priority=100):
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", semantics or clear_prohibit_semantics())
...
def test_compatible_rule_extends_canon(direct_vm, direct_deploy):
    ...
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", clear_require_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE RELATION", relation("COMPATIBLE", reason="JOINTLY_SATISFIABLE"))
...
def test_equal_priority_conflict_is_blocked_in_strict_mode(direct_vm, direct_deploy):
    ...
    direct_vm.mock_llm(
        r"RULEGRAPH / CLASSIFY RULE RELATION",
...
def test_priority_resolves_conflict_deterministically(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", clear_permit_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE RELATION", relation("CONFLICT", "MODAL"))
    second = contract.propose_rule(
        book_id,
        "During an active exploit the security council may execute a withdrawal without three approvals.",
        200,
...
def test_supersession_is_atomic_and_preserves_lineage(direct_vm, direct_deploy):
    ...
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", replacement_semantics)
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE RELATION", relation("SPECIALIZES", reason="THRESHOLD_AMENDMENT"))
    second = contract.propose_rule(
        book_id,
...
def test_unrelated_declared_supersession_is_blocked(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", clear_require_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE RELATION", relation("UNRELATED", reason="NO_SHARED_NORM"))
    second = contract.propose_rule(
        book_id,
...
def test_relation_is_pinned_to_semantic_hashes(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", clear_require_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE RELATION", relation("COMPATIBLE"))
...
def test_validator_can_reject_unfaithful_normalization(direct_vm, direct_deploy):
    ...
    bad["modality"] = "PERMIT"
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", bad)
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY NORMALIZE RULE", clear_prohibit_semantics())
...
def test_validator_accepts_faithful_normalization(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY NORMALIZE RULE", clear_prohibit_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / COMPARE INDEPENDENT NORMALIZATIONS", {"equivalent": True})
...
def test_validator_can_reject_wrong_relation(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", clear_permit_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE RELATION", relation("COMPATIBLE", reason="FALSE_COMPATIBILITY"))
    contract.propose_rule(
        book_id,
        "During an active exploit the security council may execute a withdrawal without three approvals.",
        100,
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY NORMALIZE RULE", clear_permit_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY CLASSIFY RULE RELATION", relation("CONFLICT", "MODAL"))
    assert direct_vm.run_validator() is False
...
def test_nested_supersession_repeal_restore_preserves_lineage_and_rejects_cycle(direct_vm, direct_deploy):
    ...
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", second_semantics)
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE", relation("SPECIALIZES"))
    ...
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", third_semantics)
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE", relation("SPECIALIZES"))
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE", relation("SPECIALIZES"))
...
def test_blocked_rule_receives_edges_from_rules_added_later(direct_vm, direct_deploy):
    ...
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", clear_permit_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE", relation("CONFLICT", "MODAL"))
    ...
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", clear_require_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE", relation("COMPATIBLE", reason="LATER_RULE"))
...
def test_relation_lookup_is_symmetric(direct_vm, direct_deploy):
    ...
    first = propose_first(direct_vm, contract, book_id)
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", clear_require_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE", relation("COMPATIBLE"))
...
def test_malicious_normalization_leader_is_rejected_behaviorally(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY NORMALIZE RULE", clear_require_semantics())
    assert direct_vm.run_validator() is False
...
def test_validator_rejects_conflict_when_independent_relation_is_compatible(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY NORMALIZE RULE", clear_permit_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY CLASSIFY RULE RELATION", relation("COMPATIBLE"))
    assert direct_vm.run_validator() is False
...
def test_validator_accepts_same_relation_facts_with_different_explanations(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", clear_permit_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE RELATION", relation("CONFLICT", "MODAL", "LEADER_REASON", "leader wording"))
    contract.propose_rule(book_id, "An emergency withdrawal may bypass approval.", 100, 0)
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY NORMALIZE RULE", clear_permit_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY CLASSIFY RULE RELATION", relation("CONFLICT", "MODAL", "VALIDATOR_REASON", "validator wording"))
    assert direct_vm.run_validator() is True
...
def test_validator_accepts_conflict_subtype_disagreement_as_metadata(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY NORMALIZE RULE", clear_permit_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY CLASSIFY RULE", relation("CONFLICT", "AUTHORITY"))
    assert direct_vm.run_validator() is True
...
def test_ambiguous_independent_relation_rejects_confident_leader(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY NORMALIZE RULE", clear_permit_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY CLASSIFY RULE RELATION", relation("AMBIGUOUS"))
    assert direct_vm.run_validator() is False
...
def test_normalization_validator_rejects_conservative_ambiguous_leader(direct_vm, direct_deploy):
    ...
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", ambiguous)
...
def test_normalization_validator_accepts_shared_ambiguity(direct_vm, direct_deploy):
    ...
    contract.propose_rule(book_id, "A treasury withdrawal rule.", 100, 0)
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY NORMALIZE RULE", ambiguous)
    assert direct_vm.run_validator() is True
...
def test_normalization_validator_rejects_omitted_condition(direct_vm, direct_deploy):
    ...
    leader["condition"] = ""
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", leader)
...
def test_normalization_validator_allows_equivalent_wording(direct_vm, direct_deploy):
    ...
    contract.propose_rule(book_id, "A withdrawal must not execute without three approvals.", 100, 0)
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / INDEPENDENTLY NORMALIZE RULE", independent)
    direct_vm.mock_llm(r"RULEGRAPH / COMPARE INDEPENDENT NORMALIZATIONS", {"equivalent": True})
    assert direct_vm.run_validator() is True
...
def test_malicious_relation_leader_is_rejected_behaviorally(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", clear_permit_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE", relation("COMPATIBLE", reason="IGNORE_ALL_RULES"))
...
def test_malformed_and_oversized_model_outputs_fail_closed(direct_vm, direct_deploy):
    contract, book_id = deploy_book(direct_deploy)
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", [])
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", oversized)
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE", relation("COMPATIBLE"))
...
def test_invalid_relation_subtypes_fail_closed(direct_vm, direct_deploy):
    ...
    direct_vm.clear_mocks()
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", clear_require_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE", relation("CONFLICT", "NOT_A_REAL_SUBTYPE"))
...
def test_rulebook_bound_rejects_rule_25_before_consensus(direct_vm, direct_deploy):
    contract, book_id = deploy_book(direct_deploy)
    direct_vm.mock_llm(r"RULEGRAPH / NORMALIZE RULE", ambiguous_semantics())
    direct_vm.mock_llm(r"RULEGRAPH / CLASSIFY RULE", relation("AMBIGUOUS"))
