...
CONTRACT = ROOT / "contracts" / "rule_graph.py"
...
def _load_contract():
    _install_stub()
    spec = importlib.util.spec_from_file_location("rule_graph_contract", CONTRACT)
...
def _ast_checks(source: str) -> list[str]:
    ...
    required_classes = {"RuleGraph", "Rulebook", "Rule", "Relation", "IRuleGraph"}
    required_methods = {
    ...
    assert "RULEGRAPH / INDEPENDENTLY NORMALIZE RULE" in source
    assert "RULEGRAPH / COMPARE INDEPENDENT NORMALIZATIONS" in source
    assert "RULEGRAPH / INDEPENDENTLY CLASSIFY RULE RELATION" in source
...
def main() -> int:
    ...
    print(f"RuleGraph offline preflight: {len(checks)}/{len(checks)} checks passed")
    for index, check in enumerate(checks, 1):
        print(f"  {index:02d}. PASS - {check}")
    print("\nGenVM consensus/runtime behavior is covered separately by tests/test_rule_graph.py.")
    return 0
