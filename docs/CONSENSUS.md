...
Rule_Graph therefore uses two bounded consensus calls with `gl.vm.run_nondet_unsafe`.
...
Rule_Graph still treats prompt injection as a model-layer risk, which is why outputs are bounded, validator-reviewed, and unable to choose priority.
