...
Residual limitation: historical repealed rules are excluded because Rule_Graph has no repeal-restoration operation.
...
Rule_Graph does not decide whether a real-world action complied with a rule, move funds, execute governance proposals, infer external legal authority, prove submitted text is legally authoritative, or replace legal review. Those concerns belong to consuming applications or other primitives.
