...
RULE_ACTIVE, RULE_BLOCKED, RULE_REPEALED, RULE_SUPERSEDED = 1, 2, 3, 4
MODALITY_REQUIRE, MODALITY_PERMIT, MODALITY_PROHIBIT, MODALITY_UNKNOWN = 1, 2, 3, 4
SEMANTIC_CLEAR, SEMANTIC_AMBIGUOUS = 1, 2
REL_UNRELATED, REL_COMPATIBLE, REL_REDUNDANT, REL_SPECIALIZES, REL_CONFLICT, REL_AMBIGUOUS = 1, 2, 3, 4, 5, 6
CONFLICT_NONE, CONFLICT_MODAL, CONFLICT_CONDITION, CONFLICT_EXCEPTION, CONFLICT_SCOPE, CONFLICT_AUTHORITY, CONFLICT_OTHER = 0, 1, 2, 3, 4, 5, 6
RES_NONE, RES_LEFT_PREVAILS, RES_RIGHT_PREVAILS, RES_UNRESOLVED = 0, 1, 2, 3
MAX_RULES_PER_BOOK = 24
MAX_NAME_LEN, MAX_PURPOSE_LEN, MAX_RULE_TEXT_LEN = 96, 1200, 2400
MAX_SEMANTIC_FIELD, MAX_RELATION_NOTE, MAX_REASON_CODE, MAX_PRIORITY = 320, 360, 80, 1000
ERR_EXPECTED = "EXPECTED"
MODALITY_NAMES = {MODALITY_REQUIRE: "REQUIRE", MODALITY_PERMIT: "PERMIT", MODALITY_PROHIBIT: "PROHIBIT", MODALITY_UNKNOWN: "UNKNOWN"}
SEMANTIC_NAMES = {SEMANTIC_CLEAR: "CLEAR", SEMANTIC_AMBIGUOUS: "AMBIGUOUS"}
STATUS_NAMES = {RULE_ACTIVE: "ACTIVE", RULE_BLOCKED: "BLOCKED", RULE_REPEALED: "REPEALED", RULE_SUPERSEDED: "SUPERSEDED"}
RELATION_NAMES = {REL_UNRELATED: "UNRELATED", REL_COMPATIBLE: "COMPATIBLE", REL_REDUNDANT: "REDUNDANT", REL_SPECIALIZES: "SPECIALIZES", REL_CONFLICT: "CONFLICT", REL_AMBIGUOUS: "AMBIGUOUS"}
CONFLICT_NAMES = {CONFLICT_NONE: "NONE", CONFLICT_MODAL: "MODAL", CONFLICT_CONDITION: "CONDITION", CONFLICT_EXCEPTION: "EXCEPTION", CONFLICT_SCOPE: "SCOPE", CONFLICT_AUTHORITY: "AUTHORITY", CONFLICT_OTHER: "OTHER"}
RESOLUTION_NAMES = {RES_NONE: "NONE", RES_LEFT_PREVAILS: "LEFT_PREVAILS", RES_RIGHT_PREVAILS: "RIGHT_PREVAILS", RES_UNRESOLVED: "UNRESOLVED"}
KIND_BY_NAME = {n: v for v, n in RELATION_NAMES.items()}
CONFLICT_BY_NAME = {n: v for v, n in CONFLICT_NAMES.items()}
MODALITY_BY_NAME = {"REQUIRE": MODALITY_REQUIRE, "PERMIT": MODALITY_PERMIT, "PROHIBIT": MODALITY_PROHIBIT}
...
def message_timestamp() -> int:
    raw = getattr(getattr(getattr(gl, "message", None), "raw", None), "datetime", None)
    if raw in (None, ""):
        mapping = getattr(gl, "message_raw", None)
        raw = mapping.get("datetime", "") if isinstance(mapping, dict) else ""
    if isinstance(raw, int): return int(raw)

    if not isinstance(raw, str) or raw.strip() == "": raise ValueError("transaction timestamp is unavailable")
    parsed = datetime.fromisoformat(raw.strip().replace("Z", "+00:00"))
    if parsed.tzinfo is None:
        parsed = parsed.replace(tzinfo=timezone.utc)
    return int(parsed.timestamp())
def semantic_payload(value: dict) -> dict:
    return {k: (int(value[k]) if k in ("modality", "semantic_state") else str(value[k])) for k in (
        "modality", "actor", "action", "object", "condition", "exception", "scope", "semantic_state", "ambiguity_reason"
    )}
...
def canonical_semantics(raw) -> dict:
    ...
    state = SEMANTIC_CLEAR if str(raw.get("ambiguity", "AMBIGUOUS")).strip().upper() == "CLEAR" else SEMANTIC_AMBIGUOUS
    fields = {k: str(raw.get(k, "")) for k in ("actor", "action", "object", "condition", "exception", "scope", "ambiguity_reason")}
    actor, action = bounded(fields["actor"], MAX_SEMANTIC_FIELD), bounded(fields["action"], MAX_SEMANTIC_FIELD)
    reason = bounded(fields["ambiguity_reason"], MAX_SEMANTIC_FIELD)
    if any(len(v) > MAX_SEMANTIC_FIELD for v in fields.values()):
        state, reason = SEMANTIC_AMBIGUOUS, "OVERSIZED_SEMANTIC_FIELD"
    if raw.get("atomic") is not True:
        state = SEMANTIC_AMBIGUOUS
        reason = reason or "NON_ATOMIC_OR_UNCLEAR"
    if modality == MODALITY_UNKNOWN or not actor or not action:
        state = SEMANTIC_AMBIGUOUS
        reason = reason or "MISSING_CORE_SEMANTICS"
    result = {
        "modality": modality, "actor": actor, "action": action,
        "object": bounded(fields["object"], MAX_SEMANTIC_FIELD),
        "condition": bounded(fields["condition"], MAX_SEMANTIC_FIELD),
        "exception": bounded(fields["exception"], MAX_SEMANTIC_FIELD),
        "scope": bounded(fields["scope"], MAX_SEMANTIC_FIELD),
        "semantic_state": state, "ambiguity_reason": reason}
    result["semantic_hash"] = semantic_hash(result)
    return result
def valid_semantics_shape(value) -> bool:

    if not isinstance(value, dict): return False
    if value.get("modality") not in (MODALITY_REQUIRE, MODALITY_PERMIT, MODALITY_PROHIBIT, MODALITY_UNKNOWN): return False

    if value.get("semantic_state") not in (SEMANTIC_CLEAR, SEMANTIC_AMBIGUOUS): return False
    for key in ("actor", "action", "object", "condition", "exception", "scope", "ambiguity_reason"):
        item = value.get(key)

        if not isinstance(item, str) or len(item) > MAX_SEMANTIC_FIELD: return False
    digest = value.get("semantic_hash")

    if not isinstance(digest, str) or len(digest) != 64: return False
    try:
...
def canonical_relation(raw) -> dict:
    ...
    raw_conflict = str(raw.get("conflict_type", "OTHER")).strip().upper()
    kind, conflict = KIND_BY_NAME.get(raw_kind, REL_AMBIGUOUS), CONFLICT_BY_NAME.get(raw_conflict, CONFLICT_OTHER)
    if raw_kind not in KIND_BY_NAME or (raw_kind == "CONFLICT" and raw_conflict not in CONFLICT_BY_NAME):
        kind, conflict = REL_AMBIGUOUS, CONFLICT_NONE
    if kind != REL_CONFLICT:
        conflict = CONFLICT_NONE
    return {
        "kind": kind, "conflict_type": conflict,
        "overlap": bounded(str(raw.get("overlap", "")), MAX_RELATION_NOTE),
        "reason_code": bounded(str(raw.get("reason_code", "UNSPECIFIED")), MAX_REASON_CODE).upper() or "UNSPECIFIED"}
def valid_relation_shape(value) -> bool:

    if not isinstance(value, dict): return False
    if value.get("kind") not in RELATION_NAMES or value.get("conflict_type") not in CONFLICT_NAMES: return False

    if value.get("kind") != REL_CONFLICT and value.get("conflict_type") != CONFLICT_NONE: return False
    overlap, reason = value.get("overlap"), value.get("reason_code")
...
def pair_key(rulebook_id: int, rule_a: int, rule_b: int) -> str:
    return f"{int(rulebook_id)}:{min(int(rule_a), int(rule_b))}:{max(int(rule_a), int(rule_b))}"
def build_normalize_prompt(rulebook_purpose: str, rule_text: str) -> str:
    return f"""RULEGRAPH / NORMALIZE RULE
...
{rule_text}
---END RULE---
A valid RuleGraph rule must express ONE atomic normative proposition. If the text contains multiple independent requirements, permissions, or prohibitions that should be separate graph nodes, mark atomic=false and ambiguity=AMBIGUOUS.
...
def build_independent_normalize_prompt(rulebook_purpose: str, rule_text: str) -> str:
    return build_normalize_prompt(rulebook_purpose, rule_text).replace(
        "RULEGRAPH / NORMALIZE RULE", "RULEGRAPH / INDEPENDENTLY NORMALIZE RULE", 1
    )
def build_compare_semantics_prompt(rulebook_purpose: str, rule_text: str, leader: dict, independent: dict) -> str:
    return f"""RULEGRAPH / COMPARE INDEPENDENT NORMALIZATIONS
...
Ignore harmless wording differences, but reject differences in modality, atomic clarity, actor, action, object, material condition, explicit exception, or scope.
RULEBOOK PURPOSE
---BEGIN PURPOSE---
{rulebook_purpose}
---END PURPOSE---
...
def build_relation_prompt(purpose: str, left: dict, right: dict) -> str:
    return f"""RULEGRAPH / CLASSIFY RULE RELATION
...
Priority is NOT part of this semantic judgement. Never decide which rule wins. RuleGraph resolves precedence later with deterministic protocol state.
Return JSON only:
...
def build_independent_relation_prompt(purpose: str, left: dict, right: dict) -> str:
    return build_relation_prompt(purpose, left, right).replace(
        "RULEGRAPH / CLASSIFY RULE RELATION", "RULEGRAPH / INDEPENDENTLY CLASSIFY RULE RELATION", 1
    )
...
@dataclass
class Rule:
    rule_id: u256; rulebook_id: u256; proposer: Address
    text: str; text_hash: str; modality: u8
    actor: str; action: str; object: str; condition: str; exception: str; scope: str
    semantic_state: u8; ambiguity_reason: str; semantic_hash: str
    priority: u32; status: u8; added_revision: u32; activated_version: u32; created_at: u256
    supersedes_rule_id: u256; superseded_by_rule_id: u256; relation_ids: DynArray[u256]

@allow_storage
@dataclass
class Relation:
    relation_id: u256; rulebook_id: u256; left_rule_id: u256; right_rule_id: u256
    kind: u8; conflict_type: u8; overlap: str; reason_code: str
    resolution: u8; analyzed_at: u256; left_semantic_hash: str; right_semantic_hash: str
...
class RuleGraph(gl.Contract):
    rulebooks: TreeMap[u256, Rulebook]; rules: TreeMap[u256, Rule]
    relations: TreeMap[u256, Relation]; relation_lookup: TreeMap[str, u256]
    next_rulebook_id: u256; next_rule_id: u256; next_relation_id: u256
    def __init__(self):
        self.next_rulebook_id = u256(1); self.next_rule_id = u256(1); self.next_relation_id = u256(1)
    def _require_rulebook(self, rulebook_id: u256) -> Rulebook:
        book = self.rulebooks.get(rulebook_id)

        if book is None: raise gl.vm.UserError(f"{ERR_EXPECTED}: unknown rulebook {rulebook_id}")
        return book
    def _require_rule(self, rule_id: u256) -> Rule:
        rule = self.rules.get(rule_id)

        if rule is None: raise gl.vm.UserError(f"{ERR_EXPECTED}: unknown rule {rule_id}")
        return rule
    def _require_relation(self, relation_id: u256) -> Relation:
        relation = self.relations.get(relation_id)

        if relation is None: raise gl.vm.UserError(f"{ERR_EXPECTED}: unknown relation {relation_id}")
        return relation
    ...
    def _rule_memory(self, rule: Rule) -> dict:
        return {"rule_id": int(rule.rule_id), "text": str(rule.text), "semantics": {
    ...
    def _normalize_rule(self, purpose: str, rule_text: str) -> dict:
        ...
        def validator(leader_result) -> bool:
            ...
            try:
                ...
                if not valid_semantics_shape(independent): return False
                if int(candidate["modality"]) != int(independent["modality"]): return False

                if int(candidate["semantic_state"]) != int(independent["semantic_state"]): return False
                if bool(str(candidate["condition"]).strip()) != bool(str(independent["condition"]).strip()): return False

                if bool(str(candidate["exception"]).strip()) != bool(str(independent["exception"]).strip()): return False

                if int(candidate["semantic_state"]) == SEMANTIC_AMBIGUOUS: return True
                comparison = gl.nondet.exec_prompt(
                    build_compare_semantics_prompt(purpose_mem, text_mem, candidate, independent), response_format="json"
                )
    ...
    def _analyze_relation(self, purpose: str, left: dict, right: dict) -> dict:
        ...
        def validator(leader_result) -> bool:
            ...
            try:
                ...
                if not valid_relation_shape(independent): return False
                if int(independent["kind"]) == REL_AMBIGUOUS: return int(candidate["kind"]) == REL_AMBIGUOUS
                return int(candidate["kind"]) == int(independent["kind"])
            except Exception:
                return False
        return gl.vm.run_nondet_unsafe(leader, validator)
    def _relation_resolution(self, relation: Relation) -> int:
        if int(relation.kind) != REL_CONFLICT: return RES_NONE
        left, right = self._require_rule(relation.left_rule_id), self._require_rule(relation.right_rule_id)
        if int(right.supersedes_rule_id) == int(left.rule_id): return RES_RIGHT_PREVAILS
        if int(left.supersedes_rule_id) == int(right.rule_id): return RES_LEFT_PREVAILS
        if int(left.priority) > int(right.priority): return RES_LEFT_PREVAILS
        if int(right.priority) > int(left.priority): return RES_RIGHT_PREVAILS
    ...
    def _relation_for_pair(self, rulebook_id: int, rule_a: int, rule_b: int):
        ...
        return None if int(relation_id) == 0 else self.relations.get(relation_id)
    def _blocking_reason(self, book: Rulebook, rule: Rule, include_blocked: bool = False) -> str:
        if int(rule.semantic_state) != SEMANTIC_CLEAR: return "SEMANTIC_AMBIGUITY"
        if int(rule.supersedes_rule_id) != 0:
            ...
            if relation is None: return "SUPERSESSION_RELATION_MISSING"
            if int(relation.kind) in (REL_UNRELATED, REL_AMBIGUOUS): return "INVALID_SUPERSESSION_RELATION"
        for other_id in book.rule_ids:

            if int(other_id) == int(rule.rule_id): continue
            other = self._require_rule(other_id)

            if int(other.status) != RULE_ACTIVE and not (include_blocked and int(other.status) == RULE_BLOCKED): continue
            relation = self._relation_for_pair(int(rule.rulebook_id), int(rule.rule_id), int(other.rule_id))
            if relation is None: return "MISSING_ACTIVE_RELATION" if int(other.status) == RULE_ACTIVE else "MISSING_RESTORATION_RELATION"

            if not include_blocked and not bool(book.strict_mode): continue
            if int(relation.kind) == REL_AMBIGUOUS: return "AMBIGUOUS_RELATION"
            if int(relation.kind) == REL_CONFLICT and int(relation.resolution) == RES_UNRESOLVED: return "UNRESOLVED_CONFLICT"
        return ""
    def _apply_supersession_if_needed(self, book: Rulebook, rule: Rule) -> None:
        ...
        if int(target.status) != RULE_ACTIVE:
            return
        target.status = u8(RULE_SUPERSEDED); target.superseded_by_rule_id = rule.rule_id
    ...
    def _refresh_book_state(self, rulebook_id: u256) -> None:
        ...
        active_count = blocked_count = unresolved = ambiguous = resolved = 0
        active_payload = []
        for rule_id in book.rule_ids:
            ...
            if int(rule.status) == RULE_ACTIVE:
                ...
                active_payload.append({
                    "rule_id": int(rule.rule_id), "text_hash": str(rule.text_hash),
                    "semantic_hash": str(rule.semantic_hash), "priority": int(rule.priority),
                    "supersedes_rule_id": int(rule.supersedes_rule_id)})
            elif int(rule.status) == RULE_BLOCKED:
                blocked_count += 1
        active_ids = {str(item["rule_id"]) for item in active_payload}
        relation_payload = []
        for relation_id in book.relation_ids:
            relation = self._require_relation(relation_id)

            if str(int(relation.left_rule_id)) not in active_ids or str(int(relation.right_rule_id)) not in active_ids: continue
            relation_payload.append({
                "relation_id": int(relation.relation_id), "left": int(relation.left_rule_id),
                "right": int(relation.right_rule_id), "kind": int(relation.kind),
            ...
            if int(relation.kind) == REL_CONFLICT:
                unresolved += int(relation.resolution) == RES_UNRESOLVED
    ...
    def _store_relation(self, book: Rulebook, left_rule: Rule, right_rule: Rule, outcome: dict, now: int) -> u256:
        ...
        if int(self.relation_lookup.get(key, u256(0))) != 0: raise gl.vm.UserError(f"{ERR_EXPECTED}: relation already exists")
        ...
        relation.relation_id = relation_id; relation.rulebook_id = left_rule.rulebook_id
        relation.left_rule_id = left_rule.rule_id; relation.right_rule_id = right_rule.rule_id
        relation.kind = u8(int(outcome["kind"])); relation.conflict_type = u8(int(outcome["conflict_type"]))
        relation.overlap = str(outcome["overlap"]); relation.reason_code = str(outcome["reason_code"])
        ...
        left_rule.relation_ids.append(relation_id); right_rule.relation_ids.append(relation_id)
    ...
    def create_rulebook(self, name: str, purpose: str, strict_mode: bool = True) -> u256:
        ...
        if len(name) == 0 or len(name) > MAX_NAME_LEN: raise gl.vm.UserError(f"{ERR_EXPECTED}: invalid rulebook name")
        if len(purpose) == 0 or len(purpose) > MAX_PURPOSE_LEN: raise gl.vm.UserError(f"{ERR_EXPECTED}: invalid rulebook purpose")
    ...
    @gl.public.write
    def propose_rule(self, rulebook_id: u256, text: str, priority: int = 100, supersedes_rule_id: int = 0) -> u256:
        book = self._require_rulebook(rulebook_id); self._require_owner(book)
        text = str(text).strip()

        if len(text) == 0 or len(text) > MAX_RULE_TEXT_LEN: raise gl.vm.UserError(f"{ERR_EXPECTED}: invalid rule text")
        if priority < 0 or priority > MAX_PRIORITY: raise gl.vm.UserError(f"{ERR_EXPECTED}: priority must be 0..{MAX_PRIORITY}")

        if len(book.rule_ids) >= MAX_RULES_PER_BOOK: raise gl.vm.UserError(f"{ERR_EXPECTED}: maximum {MAX_RULES_PER_BOOK} rules per rulebook")
        if supersedes_rule_id < 0: raise gl.vm.UserError(f"{ERR_EXPECTED}: supersedes_rule_id must be non-negative")
        supersedes = u256(supersedes_rule_id)
        if supersedes_rule_id != 0:
            target = self._require_rule(supersedes); self._same_book(target, rulebook_id)

            if int(target.status) != RULE_ACTIVE: raise gl.vm.UserError(f"{ERR_EXPECTED}: superseded rule must be active")
        semantics = self._normalize_rule(str(book.purpose), text)

        if not valid_semantics_shape(semantics): raise gl.vm.UserError(f"{ERR_EXPECTED}: invalid consensus semantic result")
        now = message_timestamp()
        rule_id = self.next_rule_id; self.next_rule_id = u256(int(self.next_rule_id) + 1)
        next_revision = int(book.revision) + 1
        rule = self.rules.get_or_insert_default(rule_id)
        rule.rule_id = rule_id; rule.rulebook_id = rulebook_id; rule.proposer = gl.message.sender_address
        rule.text = text; rule.text_hash = hash_text(text); rule.modality = u8(int(semantics["modality"]))
        rule.actor = str(semantics["actor"]); rule.action = str(semantics["action"]); rule.object = str(semantics["object"])
        rule.condition = str(semantics["condition"]); rule.exception = str(semantics["exception"]); rule.scope = str(semantics["scope"])
        rule.semantic_state = u8(int(semantics["semantic_state"])); rule.ambiguity_reason = str(semantics["ambiguity_reason"])
        rule.semantic_hash = str(semantics["semantic_hash"]); rule.priority = u32(priority); rule.status = u8(RULE_BLOCKED)
        rule.added_revision = u32(next_revision); rule.activated_version = u32(0); rule.created_at = u256(now)
        rule.supersedes_rule_id = supersedes; rule.superseded_by_rule_id = u256(0)
        book.rule_ids.append(rule_id)
        candidate_mem = self._rule_memory(rule)
        for other_id in book.rule_ids:

            if int(other_id) == int(rule_id): continue
            other = self._require_rule(other_id)

            if int(other.status) == RULE_REPEALED: continue
            outcome = self._analyze_relation(str(book.purpose), self._rule_memory(other), candidate_mem)

            if not valid_relation_shape(outcome): raise gl.vm.UserError(f"{ERR_EXPECTED}: invalid consensus relation result")
    ...
    def set_blocked_rule_priority(self, rule_id: u256, priority: int) -> None:
        ...
        if int(rule.status) != RULE_BLOCKED: raise gl.vm.UserError(f"{ERR_EXPECTED}: priority can change only while blocked")
        if priority < 0 or priority > MAX_PRIORITY: raise gl.vm.UserError(f"{ERR_EXPECTED}: priority must be 0..{MAX_PRIORITY}")
        if int(rule.priority) == priority:
            return
        rule.priority = u32(priority); book.revision = u32(int(book.revision) + 1)
        self._refresh_relation_resolutions(rule); self._refresh_book_state(rule.rulebook_id)
        PriorityUpdated(rule_id, u32(priority)).emit()
    @gl.public.write
    def activate_blocked_rule(self, rule_id: u256) -> None:
        rule = self._require_rule(rule_id); book = self._require_rulebook(rule.rulebook_id); self._require_owner(book)

        if int(rule.status) != RULE_BLOCKED: raise gl.vm.UserError(f"{ERR_EXPECTED}: rule is not blocked")
    ...
    def repeal_rule(self, rule_id: u256) -> None:
        ...
        if int(rule.status) in (RULE_REPEALED, RULE_SUPERSEDED): raise gl.vm.UserError(f"{ERR_EXPECTED}: rule is already inactive")
    ...
    def restore_superseded_rule(self, rule_id: u256) -> None:
        ...
        if int(rule.status) != RULE_SUPERSEDED: raise gl.vm.UserError(f"{ERR_EXPECTED}: rule is not superseded")
        if int(rule.superseded_by_rule_id) == 0: raise gl.vm.UserError(f"{ERR_EXPECTED}: supersession lineage missing")
        replacement = self._require_rule(rule.superseded_by_rule_id)

        if int(replacement.status) == RULE_ACTIVE: raise gl.vm.UserError(f"{ERR_EXPECTED}: replacement is still active")
        self._refresh_relation_resolutions(rule)
    ...
    def get_rule(self, rule_id: u256) -> dict:
        ...
        return {
            ...
            "semantic_state_name": semantic_state_name(int(rule.semantic_state)),
            "ambiguity_reason": str(rule.ambiguity_reason), "semantic_hash": str(rule.semantic_hash),
            "priority": int(rule.priority), "status": int(rule.status),
    ...
    def get_relation(self, relation_id: u256) -> dict:
        ...
        return {
            ...
            "conflict_type_name": conflict_type_name(int(relation.conflict_type)),
    ...
    @gl.public.view
    def relation_between(self, left_rule_id: u256, right_rule_id: u256) -> dict:
        left, right = self._require_rule(left_rule_id), self._require_rule(right_rule_id)

        if int(left.rulebook_id) != int(right.rulebook_id): raise gl.vm.UserError(f"{ERR_EXPECTED}: rules belong to different rulebooks")
        relation = self._relation_for_pair(int(left.rulebook_id), int(left_rule_id), int(right_rule_id))

        if relation is None: return {"exists": False}
    ...
    def get_canon(self, rulebook_id: u256) -> list[dict]:
        ...
        for rule_id in book.rule_ids:
            ...
            if int(rule.status) != RULE_ACTIVE: continue
            result.append({
    ...
    def get_canon_relations(self, rulebook_id: u256) -> list[dict]:
        ...
        for relation_id in book.relation_ids:
            ...
            if int(relation.left_rule_id) not in active or int(relation.right_rule_id) not in active: continue
            result.append({
    ...
    def blocking_reason(self, rule_id: u256) -> str:
        ...
        if int(rule.status) != RULE_BLOCKED: return ""
        return self._blocking_reason(self._require_rulebook(rule.rulebook_id), rule)
    @gl.public.view
    def is_consistent(self, rulebook_id: u256) -> bool:
